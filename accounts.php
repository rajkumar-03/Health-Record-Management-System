   ...
   <script src='data-table/buttons.print.min.js'></script>
    ...
    $('#example').DataTable( {
 	...
 	<!-- requried-jsfiles-for owl -->
        ...
        <tbody>
        	 <?php   $sqlmember ="SELECT * FROM tbl_petients WHERE Status='Accounts' ORDER BY id DESC ";
			  	 				 ...
			  	 				 $aged=$todayyear-$bornyear;
									  ...
									  if($numc!=0){
												   ...
												   $labcost=0;
													   ...
													   $cost='';$drug='';$total=0;
								   $sqld ="SELECT * FROM tbl_treatment WHERE Patientid='$id' && Date='$date' && Status!='Closed' ";
									  ...
									  if($numd!=0){     
                                                      while($found = mysqli_fetch_array($retrievesd))
	                                                 {
														  ...
														  $sqluser ="SELECT * FROM tbl_drugs WHERE Name='$drugname' ";
															  ...
															  // $date=date('y-m-d');
                                                             $sqldx ="SELECT * FROM tbl_transactions WHERE Patientid='$id' && Date='$date' && Drugname='$drugname' ";
														   ...
														   $drug=$drug.$drugname.' ('.$morning.') '.$amo.' ('.$after.') ('.$units.') '.$ddays.' days<br>';
				  	                                        $total=$total+$labcost+$costs;
														   $labc='Lab tests fee = MK'.$labcost.'.00';
                                                        }
                                                       if( $status=="Paid" || $status==""){$button='btn-success'; $btnz=1;}else{$button='btn-danger';$btnz=0;}
				        ...
				        echo"<tr>  
			                 ...
			                 <td>
			                   <a data-id='$id'  class='open-Cash btn  btn-info' title='pay by cash' ><span class='fa fa-money' style='color:white;'></span></a>
			                 	...
			                 	echo"<td>Paid</td><td>Paid</td>";
			                 }	
			                 	
			                 echo"<td>
					        <a  data-toggle='modal' href='#displaystudentinfo' data-pc='$status' data-pp='$drug' data-bn='$names' data-br='$sirname' data-bl='$gender' data-bc='$dob' data-dp='$cost' data-tf='$total' data-pn='$comment' data-ad='$tests' data-ig='$results' data-id='$id' data-ik='James' data-it='kims'  data-im='kkk' data-em='$residence' data-ep='$phone' data-et='$labc' data-pm='$oficer' class='open-studentinfo btn  $button' title='Click here to view and print receipt'><span class='fa fa-file-word-o' style='color:white;' ></span></a>
